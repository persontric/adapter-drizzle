...
export class DrizzleMySQLAdapter implements Adapter {
	...
	private session_tbl:MySQLSessionTable
	private person_tbl:MySQLPersonTable
	constructor(
		db:MySqlDatabase<any, any, any>,
		session_tbl:MySQLSessionTable,
		person_tbl:MySQLPersonTable
	) {
		this.db = db
...
export type MySQLPersonTable = MySqlTableWithColumns<{
	dialect:'mysql';
	columns:{
		id:MySqlColumn<
			{
...
function session_tbl__database_session_(raw:InferSelectModel<MySQLSessionTable>):DatabaseSession {
...
function person_tbl__database_person_(raw:InferSelectModel<MySQLPersonTable>):DatabasePerson {
